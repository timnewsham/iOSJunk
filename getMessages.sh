...
sqlite3 "$FN" "SELECT datetime(message.date / 1000000000, 'unixepoch', '+31 years', '$TZ hours'), handle.id, message.is_from_me, message.text FROM message, handle WHERE message.handle_id = handle.ROWID;"
